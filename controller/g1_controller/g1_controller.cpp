...
G1Controller::G1Controller(G1TCIContainer *tci_container,
    ...
    : tci_container_(tci_container), robot_(robot),
      ...
      joint_trq_cmd_prev_(Eigen::VectorXd::Zero(g1::n_adof)),
      wbc_qddot_cmd_(Eigen::VectorXd::Zero(g1::n_qdot)),
      ...
      init_joint_pos_(Eigen::VectorXd::Zero(g1::n_adof)), ihwbc_(nullptr),
      joint_integrator_(nullptr), wbic_(nullptr) {
  util::PrettyConstructor(2, "G1Controller");
  sp_ = G1StateProvider::GetStateProvider();

#if B_USE_MATLOGGER
  logger_ = XBot::MatLogger2::MakeLogger("/tmp/g1_controller_data");
  ...
  try {
    ...
    if (wbc_type == "ihwbc") {
      ...
      joint_integrator_ = new JointIntegrator(
          g1::n_adof, sp_->servo_dt_, jpos_lb, jpos_ub, jvel_lb, jvel_ub);
...
void G1Controller::GetCommand(void *command) {
  if (sp_->state_ == g1_states::kInitialize ||
      sp_->state_ == g1_states_wbic::kInitialize) {
  ...
  // copy command to G1Command class
...
void G1Controller::_SaveData() {
  ...
  logger_->add("state", sp_->state_);       // g1 state machine indicator
  ...
  if (sp_->state_ != g1_states::kInitialize ||
    ...
    // ICP data
    logger_->add("des_icp", static_cast<G1CoMXYTask *>(
                                tci_container_->task_map_["com_xy_task"])
                                ->des_icp_);
    logger_->add("des_icp_dot", static_cast<G1CoMXYTask *>(
                                    tci_container_->task_map_["com_xy_task"])
                                    ->des_icp_dot_);
    logger_->add("act_icp", static_cast<G1CoMXYTask *>(
                                tci_container_->task_map_["com_xy_task"])
                                ->icp_);
    ...
    // logger_->add("wbo_ang_vel_gt", static_cast<G1WBOTask *>(
    // tci_container_->task_map_["wbo_task"])
    //->local_wbo_ang_vel_gt_);
    // logger_->add("wbo_ang_vel_est", static_cast<G1WBOTask *>(
    // tci_container_->task_map_["wbo_task"])
    //->local_wbo_ang_vel_est_);
    // logger_->add(
    //"centroidal_ang_mom_gt",
    // static_cast<G1WBOTask *>(tci_container_->task_map_["wbo_task"])
