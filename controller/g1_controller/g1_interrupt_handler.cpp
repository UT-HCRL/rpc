...
#if B_USE_HPIPM
#include "controller/g1_controller/g1_state_machines_wbic/mpc_locomotion.hpp"
#endif

G1InterruptHandler::G1InterruptHandler(
    G1ControlArchitecture *ctrl_arch)
    : InterruptHandler(), ctrl_arch_(ctrl_arch), ctrl_arch_wbic_(nullptr) {
  util::PrettyConstructor(1, "G1InterruptHandler");
}

G1InterruptHandler::G1InterruptHandler(
    G1ControlArchitecture_WBIC *ctrl_arch)
    : InterruptHandler(), ctrl_arch_wbic_(ctrl_arch), ctrl_arch_(nullptr) {
  util::PrettyConstructor(1, "G1InterruptHandler WBIC");
}

void G1InterruptHandler::Process() {
  if (ctrl_arch_) {
    //======================================================================
    // IHWBC
    //======================================================================
    ...
    if (b_button_one) {
      ...
      if (ctrl_arch_wbic_->locostate() ==
          g1_states_wbic::kDoubleSupportBalance)
    ...
    if (b_button_x) {
      ...
      std::cout << "-----------------------------------" << std::endl;
      if (ctrl_arch_wbic_->locostate() == g1_states_wbic::kMPCLocomotion) {
        static_cast<MPCLocomotion_WBIC *>(
            ctrl_arch_wbic_->locomotion_state_machine_container()
                [g1_states_wbic::kMPCLocomotion])
